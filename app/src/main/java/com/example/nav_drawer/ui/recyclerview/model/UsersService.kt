package com.example.nav_drawer.ui.recyclerview.model
...
class UsersService {
    ...
    fun deleteUser(user: User) {
        val idxToDelete = users.indexOfFirst { it.id == user.id }
        if (idxToDelete != -1) {
            users.removeAt(idxToDelete)
            notifyChanges()
        }
    }

    fun moveUser(user: User, moveBy: Int) {
        val oldIdx = users.indexOfFirst { it.id == user.id }
        if (oldIdx == -1) return

        val newIdx = oldIdx + moveBy
        if (newIdx < 0 ||newIdx >=users.size) return

        Collections.swap(users, oldIdx, newIdx)
        notifyChanges()
    }
